...
\begin{frame}
  \frametitle{Per-Term Na\"{i}ve Bayes Models (2)}
  \begin{itemize}
...
\begin{frame}
  ...
  \item Random subsets of ChEMBL GPCR collection (1000 compounds)
  \item Random subsets of ChEMBL Kinase collection (1000 compounds)
\vskip 2em
\item In the Pubchem fingerprint space, the libraries are not very different
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Test Libraries}
  \includegraphics[width=\textwidth]{img-psetbs.pdf}  
\end{frame}


\begin{frame}
  \frametitle{Applying it to Some Libraries}
  \begin{itemize}
  \item Workflow starts with an individual compound - will it be
    active in the model for term $T_i$?
  \item Find the top $k$ terms that predict an active class
  \item For a library with $N_c$ compounds, we will have $kN_c$ terms
  \end{itemize}
\vskip 1em
  \centering{
    \parbox{0.75\textwidth}{\textbf{We take the top $k$ most frequent terms as
